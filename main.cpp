...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	//make prograam 
	cl_program program = load_program(context, "..\\Erosion.cl");
	if(program == 0) 
	{
	...
	//create kernel
	cl_kernel erosion = clCreateKernel(program, "erosion", 0);
	if(erosion == 0)
	{
	...
	clSetKernelArg(erosion, 0, sizeof(cl_mem), &cl_img_in);
	clSetKernelArg(erosion, 1, sizeof(cl_mem), &cl_img_out);
	clSetKernelArg(erosion, 2, sizeof(unsigned int), &DATA_SIZE);
...
/*	for ( int xx = 0; xx < 720 * 576 ; xx++)
	...
	size_t work_size = 1;
	cl_event event;
	err = clEnqueueNDRangeKernel(queue, erosion, 1, 0, &work_size, 0, 0, 0, &event);
	clFinish(queue);
	...
	morphologyEx(in_img, dst, MORPH_ERODE, element);
	//time count over
